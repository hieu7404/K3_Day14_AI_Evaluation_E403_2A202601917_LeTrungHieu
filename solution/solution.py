...
def _clamp_score(value: float) -> float:
    """Clamp a finite numeric score to the inclusive range [0.0, 1.0]."""
...
class RAGASEvaluator:
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        return _clamp_score(faithfulness)
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        return _clamp_score(completeness)
    ...
    def evaluate_context_precision(
        ...
        return _clamp_score(sum_precision / relevant_count)
...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        prompt = (
            "Score the answer against every rubric criterion on a 0-1 scale.\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n"
            f"Rubric: {json.dumps(rubric, ensure_ascii=False)}\n"
            "Return only JSON with a 'scores' object and a 'reasoning' string."
        )
        ...
        try:
            ...
            if not isinstance(scores, dict):
                # Accept the compact format used by the supplied mock:
                # {"accuracy": 0.8, "clarity": 0.7}
                scores = result
            reasoning = result.get("reasoning", raw_response)

            final_scores: dict[str, float] = {}
            for criterion in rubric:
                try:
                    value = float(scores.get(criterion, 0.5))
                except (TypeError, ValueError):
                    value = 0.5
                final_scores[criterion] = _clamp_score(value)
            ...
            return {
                "scores": {c: 0.5 for c in rubric.keys()},
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(response_means) >= 2:
            first_score = response_means[0]
            positional_bias = all(
                first_score - later_score > 0.1
                for later_score in response_means[1:]
            )
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def avg(res: list[EvalResult], attr: str) -> float:
            return sum(getattr(r, attr) for r in res) / len(res) if res else 0.0
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        fails = sum(1 for score in (f, r, c) if score < 0.5)
        if fails > 1:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures):
            fid = str(f.qa_pair.metadata.get("id") or f"F{i + 1:03d}")
            ftype = f.failure_type or "Unknown"
            cause = self.find_root_cause(f)
            fix = self._match_suggestion(f, suggestions, i)
            fid, ftype, cause, fix = (
                str(value).replace("|", "\\|").replace("\n", " ")
                for value in (fid, ftype, cause, fix)
            )
    ...
    def _match_suggestion(
        ...
        """Choose the most relevant available suggestion for a failure row."""
        if not suggestions:
            return ""

        keywords: tuple[str, ...]
        if failure.context_recall is not None and failure.context_recall < 0.5:
            keywords = ("retriev", "query", "chunk")
        elif failure.failure_type == "hallucination":
            keywords = ("ground", "unsupported", "hallucination")
        elif failure.failure_type in {"irrelevant", "off_topic"}:
            keywords = ("intent", "scope", "prompt")
        elif failure.failure_type == "incomplete":
            keywords = ("complete", "checklist", "condition")
        else:
            keywords = ()
        ...
        for suggestion in suggestions:
            ...
            if any(keyword in lowered for keyword in keywords):
                return suggestion
        return suggestions[min(index, len(suggestions) - 1)]
    ...
    def generate_improvement_suggestions(
        ...
        candidate_actions = [
            (
                categories.get("hallucination", 0)
                + sum(f.faithfulness < 0.5 for f in failures),
                "Strengthen grounding instructions and add an unsupported-claim checker",
            ),
            (
                categories.get("irrelevant", 0)
                + categories.get("off_topic", 0)
                + sum(f.relevance < 0.5 for f in failures),
                "Add intent and scope routing, then clarify the generation prompt",
            ),
            (
                categories.get("incomplete", 0)
                + sum(f.completeness < 0.5 for f in failures),
                "Add an answer checklist for required conditions, deadlines, and exceptions",
            ),
            (
                sum(
                    f.context_recall is not None and f.context_recall < 0.5
                    for f in failures
                ),
                "Improve query rewriting and chunking for cases with low context recall",
            ),
            (
                sum(
                    f.context_precision is not None and f.context_precision < 0.5
                    for f in failures
                ),
                "Rerank retrieved chunks so relevant evidence appears earlier",
            ),
        ]
        ranked = sorted(
        ...
        return [action for _, (_, action) in ranked[:3]]
